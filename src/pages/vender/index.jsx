...
function Venda() {
  ...
  const handleCompra = useCallback(async (event) => {
    event.preventDefault();
    try {
      const obg = {
        data_nascimento: (dataCompra.getFullYear() + "-" + ((dataCompra.getMonth() + 1)) + "-" + (dataCompra.getDate())),
        cpf_cliente: cpf,
        num_func: 9873271,
  ...
  const handleAddCarrinho = useCallback((event) => {
    const obj = {
      nome: nomeProduto,
      preco: precoProduto,
    }
    ...
    <div className="vender">
      ...
      <CNPJContainer onSubmit={handlePesquisaCodBarras}>
        <div>
          ...
          /></span>
          <button type="submit">Pesquisar</button>
          <button type="button" onClick={handleAddCarrinho}>Adicionar no carrinho</button>
        </div>
      </CNPJContainer>

      {
        produtosCarrinho.length !== 0 && (
          <Container onSubmit={handleCompra}>
            ...
            <p className="programa">Programa de Felidade Cadastrado: {programaBeneficio.nome} - {programaBeneficio.fator_desconto}% de desconto</p>
            <p className="total">Total a pagar: R$ {total*(1-(desconto/100))}</p>
