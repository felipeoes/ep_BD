...
function UpdateProgramaFidelidade() {
  ...
  const handlePesquisaPrograma = useCallback(async (event) => {
    event.preventDefault();
    try {

      api.defaults.headers.Authorization = 'Basic ZmVsaXBlOjEyM2Zhcm1h';
      const response = await api.get(`programa-beneficios/${idProgramaFidelidade}`);
  ...
  const handleUpdateProgramaFidelidade = useCallback(async (event) => {
    ...
    try {
      const obg = {
        id: idProgramaFidelidade, nome, descricao, fator_desconto: Number(fatorDesconto),
      }
      console.log(obg);

      api.defaults.headers.Authorization = 'Basic ZmVsaXBlOjEyM2Zhcm1h';
      const response = await api.put(`programa-beneficios/${idProgramaFidelidade}/`, obg);
